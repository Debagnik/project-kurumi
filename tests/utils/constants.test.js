...
describe('Constants Module', () => {
  describe('CONSTANTS object', () => {
    ...
    test('should contain ESCAPE_REGEX_REGEX that escapes regex special chars', () => {
      expect(CONSTANTS.ESCAPE_REGEX_REGEX).toBeInstanceOf(RegExp);
      expect('hello.world'.replaceAll(CONSTANTS.ESCAPE_REGEX_REGEX, String.raw`\$&`)).toBe('hello\\.world');
      expect('price$100'.replaceAll(CONSTANTS.ESCAPE_REGEX_REGEX, String.raw`\$&`)).toBe('price\\$100');
    });

    test('should contain LOGS_ESCAPE_REGEX that strips newlines', () => {
      expect(CONSTANTS.LOGS_ESCAPE_REGEX).toBeInstanceOf(RegExp);
      expect('line1\nline2\rline3'.replaceAll(CONSTANTS.LOGS_ESCAPE_REGEX, '')).toBe('line1line2line3');
    });

    test('should contain UNDERSCORE_SEPARATOR constant', () => {
      expect(CONSTANTS.UNDERSCORE_SEPARATOR).toBe('_');
    });
  });
});